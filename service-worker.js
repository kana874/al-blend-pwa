...
async function withUiPatch(response){
  ...
  const [uiPatch,presetPatch,jisDataPatch]=await Promise.all([loadPatchText(UI_PATCH),loadPatchText(PRESET_PATCH),loadPatchText(JIS_DATA_PATCH)]);
  const patch=[uiPatch,presetPatch,jisDataPatch].filter(Boolean).join('\n;\n');
